...
module FIP_OBC#(
  ...
  // 内部信号：alaph、beta、C
  logic signed[2*DATA_WIDTH+1:0] alaph_internal [0:M-1];
  ...
  generate
    for (i = 0; i < M; i = i + 1) begin: gen_alaph_row
      ...
      logic signed[DATA_WIDTH:0] A_even [0:(K/2)-1];
      logic signed[DATA_WIDTH:0] A_odd  [0:(K/2)-1];
  ...
  generate
    for (j = 0; j < N; j = j + 1) begin: gen_beta_col
      ...
      logic signed[DATA_WIDTH:0] B_even [0:(K/2)-1];
      logic signed[DATA_WIDTH:0] B_odd  [0:(K/2)-1];
  ...
  generate
    ...
    for (p = 0; p < M; p = p + 1) begin : row_loop
      for (q = 0; q < M; q = q + 1) begin : col_loop
        ...
        wire signed[DATA_WIDTH:0] midA_val [0:L-1];
        wire signed[DATA_WIDTH:0] midB_val [0:L-1];
