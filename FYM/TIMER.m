...
Fmax_data = zeros(length(sizes), length(methods));

for i = 1:length(sizes)
    for j = 1:length(methods)
        idx = strcmp(T.("M"), sizes{i}) & strcmp(T.("Method"), methods{j});
        if any(idx)
            Fmax_data(i, j) = T.("Fmax (MHz)")(idx);
        else
            Fmax_data(i, j) = NaN;
        end
    end
...
grid on;
set(gca, 'FontSize', 12);
ylim([0, max(Fmax_data(:), [], 'omitnan') * 1.2]);

x_vals = b(1).XEndPoints;
fail_groups = [3, 4];
fail_x = x_vals(fail_groups);
ymax = max(Fmax_data(:), [], 'omitnan');
